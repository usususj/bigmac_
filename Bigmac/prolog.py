...
class Prolog(object):
    ...
    def interact(self):
        ...
        while True:
            try:
                ...
                color_end = "\033[0m"
                action = input('%squery [%s]>%s ' % (color_start, "samsung", color_end))#self.asp.get_properties()["summary"], color_end))
            except EOFError:
                sys.stdout.write("\nExiting...\n")
                break
    ...
    def _emit_facts(self):
        ...
        node_subjects = nx.get_node_attributes(G, 'subject')
        print(node_subjects)
